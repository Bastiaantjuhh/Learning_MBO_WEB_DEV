...
class TwoFactorAuth
{
    ...
    public function createSecret($bits = 80, $requirecryptosecure = true)
    {
        ...
        if (
            $requirecryptosecure &&
            !$rngprovider->isCryptographicallySecure()
        ) {
            throw new TwoFactorAuthException(
                'RNG provider is not cryptographically secure'
            );
        }
    ...
    public function getCode($secret, $time = null)
    {
        ...
        $timestamp =
            "\0\0\0\0" . pack('N*', $this->getTimeSlice($this->getTime($time))); // Pack time into binary string
    ...
    public function verifyCode(
        ...
        for ($i = -$discrepancy; $i <= $discrepancy; $i++) {
            ...
            $timeslice = $this->codeEquals($this->getCode($secret, $ts), $code)
                ? $slice
                : $timeslice;
        }
    ...
    public function getQRCodeImageAsDataUri($label, $secret, $size = 200)
    {
        ...
        return 'data:' .
            ...
            ';base64,' .
            base64_encode(
                $qrcodeprovider->getQRCodeImage(
                    $this->getQRText($label, $secret),
                    $size
                )
            );
    }
    ...
    public function ensureCorrectTime(
        ...
        foreach ($timeproviders as $t) {
            if (!($t instanceof ITimeProvider)) {
                throw new TwoFactorAuthException(
                    'Object does not implement ITimeProvider'
                );
            }
            ...
            if (abs($timeprovider->getTime() - $t->getTime()) > $leniency) {
                throw new TwoFactorAuthException(
                    sprintf(
                        'Time for timeprovider is off by more than %d seconds when compared to %s',
                        $leniency,
                        get_class($t)
                    )
                );
    ...
    private function base32Decode($value)
    {
        ...
        foreach (str_split($value) as $char) {
            if ($char !== '=') {
                $buffer .= str_pad(
                    decbin(self::$_base32lookup[$char]),
                    5,
                    0,
                    STR_PAD_LEFT
                );
        ...
        $blocks = trim(
            chunk_split(substr($buffer, 0, $length - ($length % 8)), 8, ' ')
        );
